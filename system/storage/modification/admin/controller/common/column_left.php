...
class ControllerCommonColumnLeft extends Controller
{
	public function index()
	{
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/diamond')) {
				$catalog[] = array(
					'name'	   => 'Diamond',
					'href'     => $this->url->link('catalog/diamond', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'catalog/giadiamond')) {
				$catalog[] = array(
					'name'	   => 'Certified Diamond',
					'href'     => $this->url->link('catalog/giadiamond', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
