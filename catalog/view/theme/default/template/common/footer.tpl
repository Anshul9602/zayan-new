...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        function getWishlist() {
            try {
                return JSON.parse(localStorage.getItem("wishzayn")) || [];
            } catch (e) {
                return [];
            }
        }
        ...
        function saveWishlist(items) {
            localStorage.setItem("wishzayn", JSON.stringify(items));
            syncWishlistToServer(items);
            updateWishlistCount(items);
        }

        function updateWishlistCount(items) {
        ...
        function addToWishlistFromButton(btn) {
            var productId = btn.getAttribute("btnid");
            var productname = btn.getAttribute("btnname");
            var productimg = btn.getAttribute("btnimg");
            var producturl = btn.getAttribute("btnhref");
            var productprice = btn.getAttribute("btnprice") || '';
            var productsprice = btn.getAttribute("btnsprice") || '';
            var productstyle = btn.getAttribute("btnstyle") || '';
            var productdesign = btn.getAttribute("btndesign") || '';
            var productsize = btn.getAttribute("btnsize") || '';
            var productsize1 = btn.getAttribute("btnsiz") || '';
            var productwet = btn.getAttribute("btnwet") || '';
            ...
            var wishzayn = getWishlist();
            var isProductInWishlist = wishzayn.some(function (item) {
            ...
            saveWishlist(wishzayn);

            // Also store in OpenCart wishlist for logged-in customers
            if (typeof $ !== 'undefined') {
                $.post('index.php?route=account/wishlist/add', { product_id: productId });
            }

            btn.innerHTML = '<i class="fa fa-heart"></i>';
            btn.classList.remove('wishlist_link', 'wishlist_link1', 'wishlist_link2');
            alert("ITEM ADDED TO YOUR WISHLIST SUCCESSFULLY");
        }

        var wishzayn = getWishlist();
        updateWishlistCount(wishzayn);
        if (wishzayn.length) {
            syncWishlistToServer(wishzayn);
        }

        document.addEventListener('click', function (e) {
            var btn = e.target.closest('.wishlist_link, .wishlist_link1, .wishlist_link2');
            if (!btn) {
                return;
            }
            e.preventDefault();
            e.stopPropagation();
            addToWishlistFromButton(btn);
        });
    });
</script>
