  ...
  item.special
    ? `
      <span class="price-regular">
        MSRP: $${parseFloat(item.special.replace(/[,\$]/g, ""))
          .toFixed(0)
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
      </span>
    ...
    : `
      ${
        item.price
          ? `
          <span class="price-regular">
            MSRP: $${parseFloat(item.price.replace(/[,\$]/g, ""))
              .toFixed(0)
              .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
          </span>`
          : ""
    ...
    `
      ...
      $("html, body").animate(
        {
          scrollTop: $("#data-container").offset().top - 120,
        },
        300
      );
    },
...
$(document).on("click", ".wishlist_link1", function (e) {
  // Handled by global wishlist handler in footer
  e.preventDefault();
});

var fullProductData = window.fullProductData || [];

$("#clear-filters").click(function () {
  $(".custom-control-input").prop("checked", false);
  generatePage(window.fullProductData || fullProductData || window.originalData || []);
});
