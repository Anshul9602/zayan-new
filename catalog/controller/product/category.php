...
class ControllerProductCategory extends Controller
{
	public function index()
	{
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if ($wishlist_items1 !== null) {
					...
					foreach ($wishlist_items1_array as $item) {
						if (is_array($item) && isset($item['productid']) && (string)$item['productid'] === (string)$result['product_id']) {
			...
			$product_total = isset($product_total) ? (int)$product_total : sizeof($data['products']);
			...
			$data['results'] = sprintf($this->language->get('text_pagination'), ($product_total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($product_total - $limit)) ? $product_total : ((($page - 1) * $limit) + $limit), $product_total, ceil($product_total / max($limit, 1)));
